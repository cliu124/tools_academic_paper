...
field_all={'label_list','title_list','xlim_list','ylim_list','legend_list',...
  ...
  ,'print_size','print','resolution','loglog','marker_face_index','fontsize','fontsize_legend','markersize','linewidth','xticklabels_list','yticklabels_list','visible','legend_index','RGB','fig_num','color_hex'}; %%add the linewidth flag and default value is 1.5
field_default={{0},{0},0,0,{0},0,0,'test',1,0,0,1,300,[0,0],zeros(length(data)),40,40,12,1.5,{0},{0},1,0,{0},1,0};
...
%%Update 2022/04/11, This will fix the color plotting and then I can use
%%much more range of color by input Valid hexadecimal
%%color codes consist of '#' followed by three or six
%%hexadecimal digits. 
%see https://www.mathworks.com/help/matlab/creating_plots/specify-plot-colors.html
%for the color list
for i=1:length(data)
  if plot_config.color_hex
      linestyle_ind=[strfind(this_color{i},'-'),strfind(this_color{i},'.'),strfind(this_color{i},':')];
      linestyle=this_color{i}(linestyle_ind);
      color=this_color{i};
      color(linestyle_ind)=[];
      tmp=plot(data{i}.x,data{i}.y,linestyle,'color',color,'Linewidth',plot_config.linewidth,'Markersize',plot_config.markersize); hold on;
  else
